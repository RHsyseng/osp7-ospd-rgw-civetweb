...
This document will describe how to update OpenStack Platform director (OSPd) and OpenStack Puppet Modules (OPM) to support deploying RadosGW (RGW) functionality on OSP7 controller. OPSd on deployment server needs to be updated support deloying RGW fuctionality. OPM on the contollers needs to be update to support configuring RGW support 
